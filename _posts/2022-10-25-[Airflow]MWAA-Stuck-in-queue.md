...
- 실제 RunningTasks보다 더 많은 Task가 찍히고 있다.
...
worker가 오토스케일링 되는 과정 AWS 내부 인프라가 제대로 동작하지 않는 것이 합리적인 추측이겠습니다.
...
문제를 직접 해결하거나 deep dive하기는 어렵습니다. manged 서비스를 사용하는 manged 서비스를 사용하는 이상 min-worker == max-worker로 두어 오토스케일링을 하지 않는 이 방법이 최선이라 생각했습니다.
...
물론 좋은 방법은 아닙니다. 시스템 단에서 필요한 리소스를 미리 할당해두고 필요할때마다 스케일링되는 방식이 더 효율적인 방법입니다.
...
할 수 있는 기회가 왔을 때 잡으려면 지금부터 미리미리 준비해두는게 좋겠습니다. 다음 포스팅은 아마 Airflow On EKS 시리즈가 될 것 같습니다.
